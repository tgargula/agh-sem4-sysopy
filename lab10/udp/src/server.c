...
int check_username(int client_fd, struct message_t* msg, union addr_union addr, socklen_t socklen);
...
void init_threads() {
    ...
    while (true) {
        ...
        for (int i = 0; i < available; i++) {
            pthread_t acceptor;
            struct accepting_routine_args args;
            args.socklen = sizeof(union addr_union);
            args.socket_fd = events[i].data.fd;
            recvfrom(events[i].data.fd, &args.msg, sizeof(struct message_t), 0, &args.addr.sock, &args.socklen);
            pthread_create(&acceptor, NULL, accepting_routine, (void*)&args);
...
void* accepting_routine(void* args) {
    int socket_fd = ((struct accepting_routine_args*)args)->socket_fd;
    socklen_t socklen = ((struct accepting_routine_args*)args)->socklen;
    struct message_t msg = ((struct accepting_routine_args*)args)->msg;
    union addr_union addr = ((struct accepting_routine_args*)args)->addr;
    ...
    synchronized(&clients_mutex, {
        if (check_username(client_fd, &msg, addr, socklen) == -1) {
            return NULL;
        }
    ...
    sendto(client_fd, &msg, sizeof(struct message_t), 0, &addr.sock, socklen);
    return NULL;
}
        ...
        for (int i = 0; i < MAX_CLIENT_COUNT; ++i) {
            synchronized(&clients[i].mutex, {
                if (clients[i].taken) {
                    ...
                    sendto(clients[i].socket_fd, &msg, sizeof(msg), 0, &clients[i].addr.sock, clients[i].socklen);
...
void send_msg_start(struct message_t* msg, int player1, int player2) {
    ...
    sendto(clients[player1].socket_fd, msg, sizeof(struct message_t), 0, &clients[player1].addr.sock, clients[player1].socklen);

    msg->data.init.symbol = TICK2;
    strncpy(msg->data.init.name, clients[player1].name, CLIENT_NAME_LEN);
    sendto(clients[player2].socket_fd, msg, sizeof(struct message_t), 0, &clients[player2].addr.sock, clients[player2].socklen);
}
...
void send_msg_move(struct message_t* msg, int player) {
    ...
    synchronized(&clients[player].mutex, {
        sendto(clients[player].socket_fd, msg, sizeof(struct message_t), 0, &clients[player].addr.sock, clients[player].socklen);
