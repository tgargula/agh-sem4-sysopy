...
void print_time(struct tms start, struct tms end, clock_t start_t, clock_t end_t) {
...
int main(int argc, char *argv[]) {
    ...
    void* handle = dlopen("target/lib/lib_merge.so", RTLD_LAZY);
    ...
    for (int i = 2; i < argc; i++) {
        if (!strcmp(argv[i], "create_table")) {
            ...
            printf("Mergowanie plików:\n");
            start_t = times(&start);
            for (int j = 0; j < files; j++)
                merge(argv[i + j + 1], j);
            i += files;
            end_t = times(&end);
            print_time(start, end, start_t, end_t);
        }
        ...
        else if (!strcmp(argv[i], "remove_blocks")) {
            printf("Usuwanie bloków:\n");
            start_t = times(&start);
